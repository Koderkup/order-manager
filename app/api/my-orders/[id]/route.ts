...
export async function GET(
  ...
  try {
    ...
    // 1. Используем users вместо users_old
    // 2. Используем code вместо number для договора
    // 3. Маппим статусы из БД в статусы для UI
    const [orders] = await conn.execute(
      `SELECT
          o.id,
...
export async function PUT(
  ...
  try {
    ...
    // Проверяем существование заказа (используем правильную таблицу users)
    const [existingOrder] = await conn.execute(
      `SELECT o.* FROM orders o
       WHERE o.id = ? ${payload.role !== "admin" ? "AND o.client_id = ?" : ""}`,
      payload.role !== "admin" ? [orderId, userId] : [orderId]
...
export async function DELETE(
  ...
  try {
    ...
    const order = (existingOrder as any[])[0];
    // Проверяем статус (используем правильные значения из БД)
